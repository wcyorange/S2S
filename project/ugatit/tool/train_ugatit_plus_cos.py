...
class Train:
    ...
    def init(self):
        ...
        self.cos_loss = torch.nn.BCELoss()
        self.m = torch.nn.Sigmoid()
        #self.cos_loss = torch.nn.CrossEntropyLoss()
    ...
    # 对图片进行统一化处理
    def get_thum(self, image, size=(256, 256, 3), greyscale=False):
        #image = image.resize(size, Image.ANTIALIAS)
        image = image.squeeze(0).permute(1, 2, 0).cpu().detach().numpy()
        #image = image.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255.0
        image = image.astype(np.uint8)  # RGB
    ...
    # 计算图片之间的余弦距离
    def img_similarity_vectors_via_numpy(self, image1, image2):
    ...
    def update_G(self, inp):
        ...
        if lambda_idt > 0:
            ...
            idt_A, cam_bb, _ = self.model.G_A(real_B, z_bb)  # 用于域渐变插值
            idt_B, cam_aa, _ = self.model.G_B(real_A, z_aa)
            cosine_A = self.img_similarity_vectors_via_numpy(real_A, idt_A)
            cosine_B = self.img_similarity_vectors_via_numpy(real_B, idt_B)
            print('ssssssssssssssss')
            print(cosine_A)
            cosine_A = np.array(cosine_A)
            cosine_B = np.array(cosine_B)
            cosine_A = torch.Tensor(cosine_A).cuda()
            cosine_B = torch.Tensor(cosine_B).cuda()
            target = torch.empty(1).random_(1)
            loss_cos_A = self.cos_loss(self.m(cosine_A), target) * lambda_idt
            loss_cos_B = self.cos_loss(self,m(cosine_B), target) * lambda_idt
        else:
            loss_cos_A = 0
            loss_cos_B = 0
        # cycle loss
        ...
        loss_A = loss_G_A + loss_cos_A + loss_cycle_A + cam_loss_A + loss_itd_A + loss_identity_A # G_A的loss
        loss_B = loss_G_B + loss_cos_B + loss_cycle_B + cam_loss_B + loss_itd_B + loss_identity_B # G_B的loss

        loss_G = loss_A + loss_B
        loss_G.backward()
        return loss_G_A, loss_G_B, loss_cos_A, loss_cos_B, loss_cycle_A, loss_cycle_B, cam_loss_A, cam_loss_B, loss_identity_A, loss_identity_B
    ...
    def train_on_step(self, inp):
        ...
        loss_G_A, loss_G_B, loss_cos_A, loss_cos_B, loss_cycle_A, loss_cycle_B, loss_cam_A, loss_cam_B, loss_idt_A, loss_idt_B = self.update_G(inp)
        self.optim_G.step()
        ...
        return loss_G_A, loss_G_B, loss_cos_A, loss_cos_B, loss_cycle_A, loss_cycle_B, \
               loss_D_A, loss_D_B, loss_cam_A, loss_cam_B, loss_idt_A, loss_idt_B

    def train_on_epoch(self, epoch):
        loss_meters = []
        loss_names = ['G_A', 'G_B', 'Cos_A', 'Cos_B', 'Cyc_A', 'Cyc_B', 'D_A', 'D_B', 'CAM_A', 'CAM_B', 'IDT_A', 'IDT_B']
