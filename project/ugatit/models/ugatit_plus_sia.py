...
'''
UGATIT + identity preserving loss
'''
...
class Generator(nn.Module):
    ...
    def forward(self, img, z, only_encoder=False):
        e = self.encoder(img.to(self.gpu0))  # for identity preserving loss
        if only_encoder:
            return e
        x, x_class = self.cam(e.to(self.gpu1))
        z = self.mlp(z.to(self.gpu0))  # b, 256

        for i in range(self.n_blocks):
            x = self.adain_resblocks[i](x.to(self.gpu0), z.to(self.gpu0))

        x = self.decoder(x.to(self.gpu1))
        return x, x_class, e
...
class Discriminator(nn.Module):
    ...
    def forward(self, img):
        local_base = self.local_base(img.to(self.gpu0))
        local_x, local_class = self.local_cam(local_base.to(self.gpu1))
        local_x = self.local_head(local_x.to(self.gpu0))

        global_base = self.global_base(img.to(self.gpu1))
        global_x, global_class = self.global_cam(global_base.to(self.gpu0))
        global_x = self.global_head(global_x.to(self.gpu1))
        # print(local_x.shape, global_x.shape, global_class.shape)
        return local_x, local_class, global_x, global_class



class UGATITPlus(object):
    def __init__(self, args):
        super().__init__()
        self.G_A = Generator(args.inc, args.outc, args.ngf, args.use_dropout, args.n_blocks, args.z_dim)
        self.G_B = Generator(args.inc, args.outc, args.ngf, args.use_dropout, args.n_blocks, args.z_dim)
        if args.training:
            self.D_A = Discriminator(args.inc, args.ndf, args.d_layers)
            self.D_B = Discriminator(args.inc, args.ndf, args.d_layers)
        self.training = args.training
        self.z_dim = args.z_dim
        print('initialize ugatit_plus.')
    ...
    def __call__(self, inp):
        ...
        fakeB, cam_ab, real_e_ab = self.G_A(realA, z_ab)
        fakeA, cam_ba, real_e_ba = self.G_B(realB, z_ba)

        recA, _, _ = self.G_B(fakeB, z_aba)
        recB, _, _ = self.G_A(fakeA, z_bab)
